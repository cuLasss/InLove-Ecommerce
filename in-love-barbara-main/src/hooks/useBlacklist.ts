 ...
 * Hook: useBlacklist
 * 
 * Hook para gerenciar dados da blacklist de clientes
 */
...
export function useBlacklist() {
  ...
  // Buscar clientes consignados para adicionar à blacklist
  const { data: clients = [], isLoading: isLoadingClients } = useQuery({
    queryKey: ['clients-consignado'],
    queryFn: () => clientsApi.getByType('CONSIGNADO'),
  ...
  const removeFromBlacklistMutation = useMutation({
    ...
    onSuccess: () => {
      console.log('🎉 [useBlacklist] Mutation de desbloqueio bem-sucedida!')
      console.log('🔄 [useBlacklist] Invalidando queries da blacklist...')
      ...
      queryClient.removeQueries({ queryKey: ['blacklist'] })
      queryClient.refetchQueries({ queryKey: ['blacklist'] })
      ...
      console.log('✅ [useBlacklist] Queries invalidadas e refetch executado, mostrando toast...')
      toast({
    ...
    onError: (error: any) => {
      console.error('❌ [useBlacklist] Erro na mutation de desbloqueio:', error)
      toast({
  ...
  const checkClientBlacklist = async (clientId: string): Promise<boolean> => {
    try {
      const result = await blacklistApi.checkClient(clientId, 'CONSIGNADO')
      return result
    } catch (error) {
