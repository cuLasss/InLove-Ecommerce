  ...
  async uploadImage(
    ...
    try {
      ...
      if (!isAuthenticated) {
        return {
          success: false,
          error: 'Usuario nao autenticado. Faca login para fazer upload de imagens.',
        };
      }
...
export const useSimpleImageUpload = () => ({
  uploadImage: simpleImageUploadService.uploadImage.bind(simpleImageUploadService),
  deleteImage: simpleImageUploadService.deleteImage.bind(simpleImageUploadService),
});
