/**
 * Componente: Gerenciamento de Blacklist
 * 
 * Interface para gerenciar clientes bloqueados no consignado
 */
...
import { useBlacklistConsignado, type BlacklistFormData } from '@/hooks/useBlacklistConsignado'

interface BlacklistManagementProps {
  onBack?: () => void
}

export function BlacklistManagement({ onBack }: BlacklistManagementProps) {
  ...
  const { 
    ...
    isRemoving,
    refreshData
  } = useBlacklistConsignado()

  const handleAddToBlacklist = async () => {
    await addToBlacklist(formData)
    setIsAddDialogOpen(false)
    setFormData({ client_id: '', client_name: '', client_type: 'CONSIGNADO', reason: '', days_blocked: 0 })
  ...
  const handleRemoveFromBlacklist = async () => {
    ...
    console.log('🔍 [BlacklistManagement] Iniciando desbloqueio para cliente:', selectedClient)
    console.log('🔍 [BlacklistManagement] Blacklist entries antes:', blacklistEntries.map(e => ({ id: e.id, client_id: e.client_id, client_name: e.client_name, is_active: e.is_active })))
    
    try {
      await removeFromBlacklist(selectedClient)
      console.log('✅ [BlacklistManagement] Desbloqueio concluído com sucesso')
      
      // Limpar o estado do cliente selecionado
      setSelectedClient(null)
      
      // O toast já é mostrado pelo hook useBlacklist
      console.log('✅ [BlacklistManagement] Estado limpo')
    } catch (error: any) {
      console.error('❌ [BlacklistManagement] Erro no desbloqueio:', error)
      // O toast de erro já é mostrado pelo hook useBlacklist
    }
  }
              ...
              <div className="flex flex-col items-center desktop:flex-row desktop:items-center gap-3 sm:gap-3">
                ...
                <div className="flex-1 min-w-0 w-full desktop:w-auto">
                  ...
                  <p className="text-xs sm:text-sm desktop:text-base text-slate-600 leading-relaxed mt-1 text-center desktop:text-left px-2 desktop:px-0" style={{ wordBreak: 'normal', overflowWrap: 'anywhere', whiteSpace: 'normal' }}>
                    Gerencie clientes bloqueados no consignado
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center desktop:justify-end gap-2 sm:gap-3 desktop:gap-4 w-full desktop:w-auto">
            ...
            <DialogContent className="w-[95vw] max-w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                {/* Seleção de cliente consignado */}
                <div className="space-y-2">
                  <Label htmlFor="client-search" className="text-xs sm:text-sm">Selecionar Cliente Consignado</Label>
                  <div className="relative">
                    ...
                    <Input
                      id="client-search"
                      placeholder="Digite o nome do cliente consignado..."
                        ...
                        filteredClients.map((client) => (
                          ...
                          >
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
                              ...
                              <div className="text-[10px] sm:text-xs text-green-600 bg-green-50 px-2 py-1 rounded whitespace-nowrap flex-shrink-0 w-fit">
                                CONSIGNADO
                              </div>
                            </div>
                          </div>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="text-xs sm:text-sm text-yellow-800 break-words leading-tight">
                          {blacklistEntries.length > 0 
                            ? "Todos os clientes do consignado já estão bloqueados ou não há clientes cadastrados com o tipo CONSIGNADO."
                            : "Nenhum cliente do consignado encontrado. Certifique-se de que existem clientes cadastrados com o tipo CONSIGNADO."
                          }
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="space-y-2 sm:space-y-3 pt-2 sm:pt-3">
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-2 sm:p-3">
                      <div className="text-xs sm:text-sm font-medium text-green-800 break-words leading-tight">
                        ⚠️ Esta ação permitirá que o cliente volte a fazer consignações imediatamente.
                      </div>
                    </div>
                  </div>
